...
func (s *Server) ragQuerySource(ctx context.Context, p *Principal, wid, docID string, cfg map[string]any) (*aiSource, error) {
	rows, e := s.DB.Query(ctx, `SELECT d.id::text,d.version,g.id::text,g.revision FROM rag_vector_indexes i JOIN rag_index_grants g ON g.id=i.grant_id JOIN documents d ON d.id=i.document_id JOIN users owner ON owner.id=g.actor_id WHERE d.workspace_id=$1 AND ($2='' OR d.id::text=$2) AND d.deleted_at IS NULL AND madi_document_allowed($3,d.id,false) AND i.document_version=d.version AND i.status='ready' AND g.active AND g.revision=i.grant_revision AND g.provider_fingerprint=$4 AND NOT owner.disabled AND madi_document_allowed(owner.id,d.id,true) ORDER BY i.updated_at DESC LIMIT 200`, wid, docID, p.ID, ragProviderFingerprint(cfg))
	if e != nil {
		return nil, e
	}
	candidates := []aiSource{}
