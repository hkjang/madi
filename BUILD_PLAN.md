...
4. [진행] PostgreSQL 실제 통합 시험: 문서/권한/키/OIDC/AI/DB/백업 복원/ZIP 왕복/유지관리 통과, 최종 회귀 검증 예정
5. [진행] 전체 화면 브라우저 검증 및 데스크톱/모바일 캡처; 1차 오류를 수정하고 재검증
6. [진행] docs 홍보 페이지 및 가이드 완성, 최종 화면 캡처·기능 일치 검토
7. [진행] Docker 후보 이미지 빌드·실제 내부망 실행/첨부/내보내기/백업복원 통과. 최종 소스 재빌드·이미지 재반입 후 커밋/푸시/릴리즈/Pages

## 릴리즈 원칙

사용자 확정(2026-09-08): **P0~P3 전체를 구현한 후 첫 릴리즈한다.** 현재 v0.1.0 표기는 개발 중인 기반 버전이며 출시하지 않는다. 전체 범위는 FULL_SCOPE.md에서 추적하고 실제 동작과 검증 근거로 상태를 갱신한다. 미구현 항목을 로드맵으로 넘겨 첫 릴리즈를 완료 처리하지 않는다. 최종 버전은 전체 검증 후 결정하며 릴리즈 첨부 파일은 서비스 이미지 `madi:v버전`을 docker save 후 gzip한 `madi-v버전.tar.gz`만 제공한다. PostgreSQL 등 제3자 이미지는 번들하지 않는다. 압축 해제된 이미지는 소스 빌드나 패키지 설치 없이 실행되어야 한다.
...
- 서비스 공유 검증 빌드: `main-DYFyyc18.js`. 문서 async, 기본 편집·이력·개인화 회귀 통과 후 전체 메뉴 순차 시험 중.
