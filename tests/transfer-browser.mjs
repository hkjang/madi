...
try{
 ...
 await page.goto(base+'/app/import');await page.getByRole('heading',{name:'가져오기 센터',exact:true}).waitFor();await page.getByLabel('파일 선택 방식',{exact:true}).selectOption('folder');await page.locator('input[webkitdirectory]').setInputFiles(path.resolve('tests/fixtures/transfer'));await page.getByRole('button',{name:'미리보기 만들기',exact:true}).click();
 ...
 await page.goto(base+'/app/import');await page.getByRole('heading',{name:'가져오기 센터',exact:true}).waitFor();await shot('mobile-import');assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
