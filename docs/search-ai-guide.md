...
문서 관련 결과는 상위 문서와 공간을 포함한 현재 열람 권한으로 제한됩니다. 데이터베이스 검색에는 별도 `database:read` 키 권한이 필요합니다. 사용자 결과는 제한 없는 로그인 세션에서만 제공됩니다. 파일 본문 전체를 OCR·PDF 추출하여 검색한다고 표시하지 않습니다.
...
pgvector 모드는 외부 PostgreSQL에 실제 `vector` 확장이 설치되어 있어야 합니다. madi는 운영 DB에 확장을 자동 설치하지 않으며 미설치 상태에서 가짜 pgvector 결과로 대체하지 않습니다. 현재 구현은 허용된 범위 내 정확 코사인 비교이며, 대규모 HNSW·IVFFlat 성능을 검증했다고 주장하지 않습니다. 연산자와 확장 계약은 [공식 pgvector 문서](https://github.com/pgvector/pgvector)를 따릅니다.
